-- =============================================================================
-- YouInc — Row-Level Security  |  P1 migration 2/3
-- =============================================================================
-- ✅  VERIFIED (2026-07-04): RLS isolation suite passes on a local Supabase stack
--     (supabase/tests/rls_isolation.sql) — cross-tenant reads return zero, WITH
--     CHECK blocks cross-tenant writes, no memberships recursion.
--     OWNER REQUIREMENT (see §1): the SECURITY DEFINER helpers must be owned by a
--     role with BYPASSRLS. Confirmed: they are owned by `postgres`, which has
--     BYPASSRLS. Re-confirm if a future deploy runs migrations as another role.
-- =============================================================================
...
-- their own invite. Acceptance is therefore a SECURITY DEFINER RPC that looks
-- the invite up by its opaque `code`, validates it, creates the membership with
-- the invite's ROLE (tier is NOT on the invite — it comes from the tenant), and
-- marks the invite accepted — all bypassing RLS inside the function.
-- >>> The RPC is defined in migration 20260704120003_accept_invite.sql. <<<
