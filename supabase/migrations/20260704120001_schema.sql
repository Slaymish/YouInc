-- =============================================================================
-- YouInc — Multi-tenant Postgres schema (Supabase)  |  P1 migration 1/3
-- =============================================================================
-- ✅  VERIFIED (2026-07-04): applies clean via `supabase db reset` on a local
--     Supabase stack; RLS isolation suite passes (supabase/tests/rls_isolation.sql).
-- =============================================================================
...
-- Invite/approval gate. An invite grants membership (with a role) in a specific
-- tenant. TIER IS NOT ON THE INVITE: tier is a tenant-level attribute
-- (tenants.tier), so the tier an invitee lands at is whatever the target tenant
-- already carries. A concierge client is invited into a tenant that was created
-- at tier='concierge' (see accept_invite RPC + the concierge-onboarding note in
-- 20260704120003_accept_invite.sql). Acceptance is handled by a SECURITY DEFINER
-- RPC because the accepting user has no membership yet.
