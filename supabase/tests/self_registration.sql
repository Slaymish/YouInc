...
begin
  ...
  assert t.tier = 'self-serve', 'create_tenant: self-service signups are always self-serve tier';
  ...
  raise notice 'PASS: create_tenant created Dana''s self-serve tenant (slug=%)', t.slug;
end $$;
...
begin
  ...
  assert t.tier = 'self-serve', 'E create_tenant: tier self-serve';
