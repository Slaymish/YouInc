...
export function Pricing() {
  return (
    <section className="pricing" aria-labelledby="pricing-heading">
      ...
      <h2 id="pricing-heading" className="section-heading">Three ways to run on YouInc.</h2>
      <div className="pricing__grid">
        <article className="tier">
          <h3 className="tier__name">{PRICING.demo.name}</h3>
          <p className="tier__price">{PRICING.demo.price}</p>
          <ul className="tier__features">
            {PRICING.demo.features.slice(0, TEASER_FEATURE_COUNT).map((f) => <li key={f}>{f}</li>)}
          </ul>
        ...
        <article className="tier tier--featured">
          ...
          <ul className="tier__features">
            {PRICING.selfServe.features.slice(0, TEASER_FEATURE_COUNT).map((f) => <li key={f}>{f}</li>)}
          </ul>
        ...
        <article className="tier">
          ...
          <ul className="tier__features">
            {PRICING.concierge.features.slice(0, TEASER_FEATURE_COUNT).map((f) => <li key={f}>{f}</li>)}
          </ul>
